...
Append one line per post to `~/.curious-stack/telemetry.jsonl` (create if missing):

```json
{"event":"skill.run","skill":"feed-scanner","version":"1.0.0","timestamp":"<ISO 8601 now>","harness":"claude-code","outcome":{"category":"<verdict lowercase>","genre":"<genre>","input_length":<char count of that post>,"output_length":<char count of table row>,"signals_detected":<signal count>}}
```
